...
export default function ViewDitePlans() {
    ...
    let deletePlan = (e) => {
        // e.preventDefault()
        // if (e) {
        axios.delete(`${ADMIN_URL}/admin/dite/delete/${e}`)
            .then((res) => res.data)
            .then((finalData) => {
                if (finalData.status) {
                    viewPlans()
                    toast.success("Dite Plan Deleted")
                } else {
                    toast.error(finalData.message || "Dite Plan Not Deleted")
                }
                            ...
                            {plans.length > 0 ? (
                                plans.map((items, index) => (
                                    <tr key={items.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-3">{items.healthScore} / 100</td>
                                        <td className="px-6 py-3">{items.time}</td>
                                        <td className="px-6 py-3 text-center">
                                            <div className="flex items-center justify-center space-x-2">
                                                <Link to={`../add/dite/${items._id}`}>
                                                    <button
